...
var LanguageSubMenu = GObject.registerClass(class LanguageMenu extends PopupMenu.PopupSubMenuMenuItem {
	...
	_init(language, duolingo) {
		...
		this.insert_child_at_index(new St.Label({
		...
		menu_total_points.add(new St.Label({text: _('Total'), x_expand: true, style: 'font-weight: bold;'}));
		menu_total_points.add(new St.Label({text: Utils.formatThousandNumber(language[Constants.LANGUAGE_POINTS].toString()) + ' XP', style: 'font-weight: bold;'}));
		this.menu.addMenuItem(menu_total_points);

		var menu_next_level = new PopupMenu.PopupBaseMenuItem();
		menu_next_level.add(new St.Label({text: _('Next level in'), x_expand: true}));
		menu_next_level.add(new St.Label({text: Utils.formatThousandNumber(language[Constants.LANGUAGE_TO_NEXT_LEVEL].toString()) + ' XP'}));
